      ...
      _export("Out_WaitPlayerInput", Out_WaitPlayerInput = class Out_WaitPlayerInput extends (_crd && BattleBaseComp === void 0 ? (_reportPossibleCrUseOfBattleBaseComp({
        ...
        start() {
          ...
          if (this.data["isPlayerInput"] !== undefined) {
            let isInput = this.data["isPlayerInput"];
            let oldInputs = this.data["inputs"] || [];
            let newEditData;
            ...
            for (let inputData of oldInputs) {
              if (inputData.opType == (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
                ...
                for (let index = 0; index < this.copyHandCards.length; index++) {
                  let handCardData = this.copyHandCards[index];
        ...
        canMove(instId, area) {
          ...
          let editCardData = this.getEditCardData(cardData.instId);
          ...
          if (cardData.location == (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
            ...
            let stateList = (_crd && battleDataMgr === void 0 ? (_reportPossibleCrUseOfbattleDataMgr({
              error: Error()
        ...
        canDrop(instId, area) {
          ...
          if (area != undefined) {
            ...
            let areaData = (_crd && battleDataMgr === void 0 ? (_reportPossibleCrUseOfbattleDataMgr({
              error: Error()
            ...
            let dropLocation = this.getPlayerAreaEmptyCardLocation(area); //区域已满不可放置
        ...
        operateHandle(instId, area, cardState) {
          ...
          if (area != undefined) {
            let dropLocation = this.getPlayerAreaEmptyCardLocation(area); //在地形区域放下

            let editCardData = this.getEditCardData(dragLocation.card.instId);
            ...
            if (editCardData) {
              if (dragLocation.card.location == (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
                ...
                //区域上的卡放回原来区域位置
                let delIdx = this.editGridList.indexOf(editCardData);
                let delEditCardData = this.editGridList[delIdx];
            ...
            if (dragLocation.card.location == (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
              ...
              let terrainData = this.copyAreas[dropLocation.areaIdx];
              ...
              let terrainData = this.copyAreas[dragLocation.areaIdx];
        ...
        onConfirmOpInputPush(data) {
          ...
          let playerIdB = (_crd && battleDataMgr === void 0 ? (_reportPossibleCrUseOfbattleDataMgr({
            error: Error()
          }), battleDataMgr) : battleDataMgr).getPlayerId(false);
          let playerIdA = (_crd && battleDataMgr === void 0 ? (_reportPossibleCrUseOfbattleDataMgr({
            error: Error()
          }), battleDataMgr) : battleDataMgr).getPlayerId(true);
        ...
        updateHandState(isCheck = false) {
          let cardState = js.createMap();
        ...
        checkLimitDropCardCost(cardId, state) {
          ...
          let cost = this.areaLimitCardDrop[state];
        ...
        checkLimitDropCardNum(areaIdx, state) {
          let terrainData = this.copyAreas[areaIdx];
          let haceNum = terrainData.cards.length;
          let cardNum = this.areaLimitCardNum[state];
        ...
        checkLimitPlayHandNum() {
          ...
          for (let key in this.limitPlayHandNum) {
            if ((_crd && battleDataMgr === void 0 ? (_reportPossibleCrUseOfbattleDataMgr({
              error: Error()
            }), battleDataMgr) : battleDataMgr).chekPersistentStateByInstId(this.playerData.instId, Number(key))) {
              limitNum = this.limitPlayHandNum[key]; //以限制最少的状态为最终限制
