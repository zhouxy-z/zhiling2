      ...
      */
      ...
      _export("VList", VList = (_dec = ccclass('VList'), _dec2 = property({
        ...
        onLoad() {
          ...
          let temps = [];
          let cont = this.node.getChildByPath(`view/content`) || this.node;
          let contWidget = cont.getComponent(Widget);
        ...
        initRollView() {
          let view = this.node.getChildByName(`view`) || new Node(`view`);
          let viewTrans = view.getComponent(UITransform) || view.addComponent(UITransform);
          ...
          let widget = view.getComponent(Widget) || view.addComponent(Widget);
          ...
          let content = view.getChildByName(`content`) || new Node(`content`);
        ...
        onCellClick(event) {
          ...
          let cellInfo = this.getVisibleInfo(cell.name);
          if (!cellInfo) return;
        ...
        onScrolling() {
          ...
          let currentPos = this._rollView.getScrollOffset();

          let time = Date.now();
          let deltaTime = (time - this._lastRollTime) / 1000;
          let rollDir = RollViewDir.None;

          if (deltaTime > 0) {
            let deltaX = currentPos.x - this._lastContPos.x;
            let deltaY = currentPos.y - this._lastContPos.y;
            let velocityX = deltaX / deltaTime;
            let velocityY = deltaY / deltaTime; // 判断主要滚动方向
        ...
        onSelect(cellInfo, cell, isClick = false) {
          ...
          if (this.selectType == SelectType.ToggleGroup) {
            let lastSelectIndex = this._curSelectIndexList[0];
            ...
            let index = this._curSelectIndexList.indexOf(cellInfo.index);
        ...
        refreshSize() {
          ...
          let offset = this._rollView.getScrollOffset();
          ...
          if (this.notLineIsAlignCenter && this._contentTrans.width < this._rollView.view.width) {
            ...
            let cellInfo = this._tempRollIndex != -1 ? this._cellInfos[this._tempRollIndex] : null;
            this._tempRollIndex = -1;
            ...
            if (cellInfo) {
              ...
              if (this._isFirstUpdate) {
                ...
                //恢复偏移
                let maxOffset = this._rollView.getMaxScrollOffset();
        ...
        setUpdateVisibleFlag(force = false) {
          if (force) {
            this.refreshVisible();
            this.recycleInvisible();
            return;
        ...
        updateListLayou() {
          ...
          for (let index = 0; index < this._cellCount; index++) {
            ...
            cellName = (_this$getCellName = (_this$getCellName2 = this.getCellName) == null ? void 0 : _this$getCellName2.call(this, index)) != null ? _this$getCellName : this._cellDefName;
            ...
            cellSize = (_ref = (_this$_cellTempSize$g = this._cellTempSize.get(cellInfo.cellName)) != null ? _this$_cellTempSize$g : (_this$getCellSize = this.getCellSize) == null ? void 0 : _this$getCellSize.call(this, cell, index)) != null ? _ref : cell.getComponent(UITransform).contentSize;
        ...
        refreshVisible() {
          this._isUpdateVisible = false;
          let visibleCellInfos = this.getVisibleCellInfos();
          let cellInfo;
          let cell;
          let isAdd;
          ...
          for (let index = 0; index < visibleCellInfos.length; index++) {
            ...
            if (!cell) {
              isAdd = true;
              let pool = this._cellPoolMap[cellInfo.name];
              cell = pool.get();
        ...
        recycleInvisible(isForce = false) {
          this._isRecycleInvisible = false;
          this.resetVisibleRect();
          let contSize = this._contentTrans.contentSize;
          let boundingBox;
          let pool;
          let cellInfo;
          let cell;
        ...
        getVisibleCellInfos() {
          ...
          let checkIndex = -1;
          let left = 0;
          let right = this._cellInfos.length - 1;
          let index;
          let cellInfo; //根据当前容器位置，用二分叉查找法找出中位数
        ...
        scrollToIndex(index, timeInSecond = 0, attenuated = true) {
          ...
          let cellInfo = this._cellInfos[index];
        ...
        selectIndex(index, timeInSecond = 0, attenuated = true) {
          if (this.selectType != SelectType.ToggleGroup) {
      ...
      _export("RollView", RollView = class RollView extends ScrollView {
        ...
        _onTouchMoved(event, captureListeners) {
          let rollTarget = this.getRollTarget(event, captureListeners);
          ...
          if (this.node === rollTarget) {
            ...
            if (this.vList.isOpenPullHeader || this.vList.isOpenPullFooter) {
              ...
              let offset = this.vertical ? outOfBoundary.y : -outOfBoundary.x; //console.log("---------->offset")
        ...
        */
        ...
        getRollTarget(event, captureListeners) {
          let cache = event.target[`__RollTarget`];
        ...
        autoScrollToPage() {
          ...
          let pageIndex = this.vList.getSelectIndex();
          ...
          if (bounceBackStarted) {
            ...
            let moveOffset = new Vec2();
            Vec2.subtract(moveOffset, this._touchBeganPos, this._touchEndPos);
            let index = pageIndex;
            let nextIndex = index + this.getDragPageNum(moveOffset);
            let timeInSecond = 0.3 * Math.abs(index - nextIndex);
