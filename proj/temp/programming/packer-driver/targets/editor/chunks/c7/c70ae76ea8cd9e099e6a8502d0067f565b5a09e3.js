    ...
    execute: function () {
      ...
      _export("LoginModule", LoginModule = class LoginModule {
        ...
        onPlayerLoginPush(data) {
          ...
          let player_data = new (_crd && Req === void 0 ? (_reportPossibleCrUseOfReq({
            error: Error()
        ...
        async onLogin(loginHost, server, post, user) {
          console.log("登录账号", loginHost, server, post, user);
          let result = await (_crd && Http === void 0 ? (_reportPossibleCrUseOfHttp({
            error: Error()
