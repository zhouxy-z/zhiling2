    ...
    execute: function () {
      ...
      _export("RewardTips", RewardTips = class RewardTips extends (_crd && Panel === void 0 ? (_reportPossibleCrUseOfPanel({
        ...
        showGetCard() {
          ...
          let listTrans = this.cardList.node.getComponent(UITransform);
          let viewTrans = this.cardList.node.getChildByName("view").getComponent(UITransform);
          let width;
          let height;
        ...
        showGetItem() {
          ...
          let listTrans = this.itemList.node.getComponent(UITransform);
          let viewTrans = this.itemList.node.getChildByName("view").getComponent(UITransform);
          let width;
          let height;
        ...
        initVisbleItemList() {
          let cellList = this.itemList.visibleCellList;
        ...
        toNodeTween(cellList) {
          ...
          let delayTime = this.showAwardTween(cellList, 0.2, 0.1);
          this.scheduleOnce(() => {
        ...
        updateCellItem(cell, cellInfo) {
          let itemData = this.itemDatas[cellInfo.index];
          let stdItem = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
          }), CfgMgr) : CfgMgr).Getitem(itemData.id);
          let bg = cell.getChildByName("bg").getComponent(Sprite);
          let icon = cell.getChildByName("icon").getComponent(Sprite);
          let numLab = cell.getChildByName("numLab").getComponent(Label);
          let nameLab = cell.getChildByName("nameLab").getComponent(Label);
          let effect = cell.getChildByName("effect");
          effect.active = false;
        ...
        initVisbleCardList() {
          let cellList = this.cardList.visibleCellList;
          this.toNodeTween(cellList);
        }

        clickCellCardList(cell, cellInfo) {
          let cardData = this.cardDatas[cellInfo.index];
        ...
        updateCellCard(cell, cellInfo) {
          let cardData = this.cardDatas[cellInfo.index];
          let card = cell.getChildByName("Cards");
          let cardCom = card.getComponent(_crd && Card === void 0 ? (_reportPossibleCrUseOfCard({
            error: Error()
        ...
        showAwardTween(nodeList, t = 0.2, d = 0.05, s = 0, e = 1) {
          let node;
          let totalTime;
        ...
        playItemEffect(node) {
          ...
          let effect = (_node$getChildByName = node.getChildByName("effect")) == null ? void 0 : _node$getChildByName.getComponent(sp.Skeleton);
