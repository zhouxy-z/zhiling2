  ...
  return {
    ...
    execute: function () {
      ...
      _export("EffectSet", EffectSet = (_dec = ccclass('EffectSet'), _dec2 = disallowMultiple(true), _dec3 = property({
         ...
         * @returns 
         */
        async Play(callBack, targetSet, delay, copy) {
          var _this$attacker, _this$target, _targetSet$TargetType;

          this.Reset();
          if (this.type == EffectType.none) return this.playNext();
          let $attacker = (_this$attacker = this.attacker) == null ? void 0 : _this$attacker.worldPosition;
          ...
          if (!$attacker) {
            ...
            $attacker = ((_this$attacker2 = this.attacker) == null ? void 0 : _this$attacker2.worldPosition) || Vec3.ZERO;
          }
          ...
          if (this.buffEvent) {
            (_crd && EventMgr === void 0 ? (_reportPossibleCrUseOfEventMgr({
              error: Error()
            }), EventMgr) : EventMgr).on("buff_event", this.onBuffEvent, this);
          }
          ...
          if (this.par) {
            ...
            let name = this.ske.getCurrent(0).animation.name || (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
              error: Error()
            ...
            if (!this.isLoop) {
              ...
              this.duration = (((_this$ske$getCurrent = this.ske.getCurrent(0)) == null || (_this$ske$getCurrent = _this$ske$getCurrent.animation) == null ? void 0 : _this$ske$getCurrent.duration) || 0) * (this.times || 1);
            } else {
              var _this$ske$getCurrent2;

              this.duration = ((_this$ske$getCurrent2 = this.ske.getCurrent(0)) == null || (_this$ske$getCurrent2 = _this$ske$getCurrent2.animation) == null ? void 0 : _this$ske$getCurrent2.duration) || 0;
            }
          } else if (this.spr) {
          ...
          switch (this.type) {
            ...
            case EffectType.bullet:
              let $target = (_this$target = this.target) == null ? void 0 : _this$target.worldPosition;
              if (!$target) $target = (_targetSet$TargetType = targetSet[TargetType[this.targetType]]) == null ? void 0 : _targetSet$TargetType.worldPosition;
              if (!$target) return this.playNext();
              this.tempV.set($target);
              this.duration = Vec3.distance($attacker, this.tempV) / this.speed; // console.log("buttle", this.duration);

              let update = undefined;
              if (this.face) (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
                error: Error()
              }), Utils) : Utils).FaceToNode(this.node, $attacker);

              if (this.pathType != PathType.liner) {
                if (!this.bezier) {
                  if (this.pathType == PathType.curve) {
                    this.bezier = (_crd && Bezier === void 0 ? (_reportPossibleCrUseOfBezier({
                      error: Error()
                    }), Bezier) : Bezier).craeteCurve($attacker, $target, this.curveRatio);
                  } else {
                    this.bezier = (_crd && Bezier === void 0 ? (_reportPossibleCrUseOfBezier({
                      error: Error()
                    }), Bezier) : Bezier).create($attacker, $target, this.bezierPath);
                  }
                }
