    ...
    execute: function () {
      ...
      _export("DragDropMgr", DragDropMgr = class DragDropMgr {
        ...
        static onTouchBegin(event) {
          ...
          let touchNode = this.getTouchNode(event); //无触摸目标事件派发给渲染在下一层级的节点
        ...
        static onTouchMove(event) {
          if (this._isCanDrag) {
            ...
            if (!this._isDragging && this._isTouch) {
              let pos = event.getUILocation(); //拖动一定范围才算是拖拽
        ...
        static isIntersects(dargNode, dropNode) {
          let dragBox = this.getWorldBox(dargNode);
          let dropBox = this.getWorldBox(dropNode);
        ...
        static getWorldBox(node) {
          ...
          let box = new Rect(-trans.anchorPoint.x * trans.width, -trans.anchorPoint.y * trans.height, trans.width, trans.height);
        ...
        static removeDragNode(node) {
          let index = this._canDragNodeList.indexOf(node);
