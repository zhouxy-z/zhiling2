    ...
    execute: function () {
      ...
      _export("VSModelPanel", VSModelPanel = class VSModelPanel extends (_crd && Panel === void 0 ? (_reportPossibleCrUseOfPanel({
        ...
        onLoad() {
          ...
          let starElementPage = this.find("starElementPage").addComponent(_crd && VSStarElementPage === void 0 ? (_reportPossibleCrUseOfVSStarElementPage({
            error: Error()
          }), VSStarElementPage) : VSStarElementPage);
          let seasonPage = this.find("seasonPage").addComponent(_crd && VSSeasonPage === void 0 ? (_reportPossibleCrUseOfVSSeasonPage({
            error: Error()
        ...
        flush(selectType = (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
          error: Error()
        }), proto) : proto).base.BattleMode.BattleModeStar) {
          let selectIndex = 0;
          let tabData;
        ...
        updateCellTab(cell, cellInfo) {
          let tabInfo = this.tabDatas[cellInfo.index];
          cell.getChildByPath("select").active = cellInfo.isSelect;
          let bg = cell.getChildByPath("bg").getComponent(Sprite);
          let selectImg = cell.getChildByName("select").getComponent(Sprite);
        ...
        tabCondCheck(index, showTips = false) {
          let tabInfo = this.tabDatas[index];
          let result = false;
