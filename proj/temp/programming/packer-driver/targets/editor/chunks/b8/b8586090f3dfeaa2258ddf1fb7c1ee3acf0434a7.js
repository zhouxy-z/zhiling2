      ...
      _export("BattleDataMgr", BattleDataMgr = class BattleDataMgr {
        ...
        setCreatRoomData(roomData) {
          ...
          for (let key in this._creatRoomData.players) {
            let playerData = this._creatRoomData.players[key];
            let cardGroupData = playerData.deckAttr;
            let stdCardBack = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
              error: Error()
            }), CfgMgr) : CfgMgr).GetCardBackCfgById(cardGroupData == null ? void 0 : cardGroupData.cardBackId);
            let cardBackName = stdCardBack ? stdCardBack.icon1 : "bg_1";
            (_crd && FightData === void 0 ? (_reportPossibleCrUseOfFightData({
              error: Error()
        ...
        getRoomDoubleNum(addCont = 0) {
          let base = 1;
          let doubleCont = 0;

          for (let key in this._doubleData) {
            let doubleData = this._doubleData[key];
        ...
        getPlayerDoubleResidue(playerId) {
          let doubleData = this._doubleData[playerId];
        ...
        analysisBattleData(roomId, data) {
          ...
          let packData;
          let comp;
          let changeDataCall; // console.dir(data);
          ...
          switch (data.opType) {
            ...
            case (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
              ...
              let isGiveUp = data.settlement.leavePlayerId && data.settlement.leavePlayerId != "";
            ...
            case (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
              ...
              if (data.retransmissionRound.waitPlayerInput) {
                let inputData = (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
                  error: Error()
                ...
                let inputPackData = (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
                  error: Error()
                }), proto) : proto).base.BattleRoomOpOutput_WaitPlayerInput.create();
                inputPackData.timeout = data.retransmissionRound.timeout;
                let inputComp = (_crd && PanelDefine === void 0 ? (_reportPossibleCrUseOfPanelDefine({
                  error: Error()
                }), PanelDefine) : PanelDefine).Out_WaitPlayerInput;
            ...
            case (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
              ...
              let giveUp = data.retransmissionSettlement.settlement.leavePlayerId && data.retransmissionSettlement.settlement.leavePlayerId != "";
        ...
        setBattleRoomData(data) {
          ...
          let myId = (_crd && PlayerData === void 0 ? (_reportPossibleCrUseOfPlayerData({
            error: Error()
        ...
        addBattleLog() {
          ...
          let logDatas = (_crd && LocalStorage === void 0 ? (_reportPossibleCrUseOfLocalStorage({
            error: Error()
        ...
        getPlayerTerrainFormId(playerId, instId) {
          ...
          let terrains = playerData.terrain;
        ...
        getPlayerTerrainFormIdx(playerId, areaIdx) {
          let playerData = this.getPlayerData(playerId);
          let terrains = playerData.terrain[areaIdx];
          return terrains;
        }
        /**
         * 获取玩家区域列表
        ...
        getPlayerTerrainTotalPow(playerId, areaIdx) {
          let base = this.getPlayerTerrainBasePow(playerId, areaIdx);
          let cardTotalPower = this.getPlayerTerrainCardTotalPow(playerId, areaIdx);
        ...
        */
        ...
        getPlayerTerrainCardTotalPow(playerId, areaIdx, isMult = true) {
          let playerData = this.getPlayerData(playerId);
          let terrainData = playerData.terrain[areaIdx];
          let count = 0;
        ...
        getPlayerHandCard(playerId, instId) {
          let cards = this.getPlayerHandCards(playerId);
        ...
        getPlayerCardCost(playerId, instId) {
          ...
          let cardData = this.getPlayerHandCard(playerId, instId);
        ...
        getPlayerTerrainCard(playerId, instId) {
          let terrains = this.getPlayerTerrainDatas(playerId);
        ...
        getCard(instId) {
          let playerIdA = this.getPlayerId();
          let playerIdB = this.getPlayerId(false);
          let cardData = this.getPlayerCard(playerIdA, instId);
        ...
        getCardByInstId(instId, all = false) {
        ...
        chekPersistentStateByInstId(instId, buffType) {
          let traget = this.getEntityByInstId(instId);
          if (!traget || !traget.persistentState) return false;
          let state = traget.persistentState[buffType];
        ...
        changePlayerPower(playerId, data) {
          ...
          for (let key in data.terrainPower) {
            ...
            let playerData = this.getPlayerData(key);
        ...
        changePlayerEnergy(playerId, data) {
          if (!this._roomData) return;
          let playerData = this.getPlayerData(data.playerId);
        ...
        */
        ...
        changeAddCardToHand(playerId, data) {
          ...
          let playerData = this.getPlayerData(data.card.playerId);
        ...
        changeCardData(playerId, data) {
          ...
          if (data.card.location == (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
            ...
            //修改地形卡
            let terrainData = playerData.terrain[data.card.area];
        ...
        changeLockZone(playerId, data) {
          if (!this._roomData) return;
          let traget = this.getEntityByInstId(data.toInstId);
          if (!traget) return;
          ...
          for (let state of data.state) {
            let stateValue = traget.persistentState[state];
            ...
            traget.persistentState[state] = stateValue;
            let base = (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
              error: Error()
          ...
          for (let state of data.state) {
            ...
            if (stateValue && stateValue.base && stateValue.base.length) {
              for (let index = 0; index < stateValue.base.length; index++) {
                let base = stateValue.base[index];
        ...
        addCardToTerrainByIdx(playerId, areaIdx, cardData) {
          let playerData = this.getPlayerData(playerId);
          let terrains = playerData.terrain;
          let terrainData = terrains[areaIdx];
