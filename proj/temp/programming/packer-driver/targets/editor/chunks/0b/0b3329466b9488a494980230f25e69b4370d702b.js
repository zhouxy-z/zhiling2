    ...
    execute: function () {
      ...
      _export("Out_WaitPlayerInput_2d", Out_WaitPlayerInput_2d = class Out_WaitPlayerInput_2d extends (_crd && BattleBaseComp === void 0 ? (_reportPossibleCrUseOfBattleBaseComp({
        ...
        creatDragNode(node) {
          ...
          let cardData = (_crd && ObjectUtils === void 0 ? (_reportPossibleCrUseOfObjectUtils({
            error: Error()
        ...
        toDrop(dropNode, dragData) {
          ...
          let dragCardData = dragData.data;

          if (battleTerrainArea) {
            //在地形区域放下
            let editCardData = this.getEditCardData(dragCardData.cardData.instId);
            ...
            if (dragCardData.cardData.location == (_crd && base === void 0 ? (_reportPossibleCrUseOfbase({
              ...
              let terrainData = this.copyAreas[dropGrid.areaIdx];
              terrainData.cards.push(dragCardData.cardData);
              this.curEnergy -= dragCardData.cardData.cost.finalValue;
            } else if (dragCardData.cardData.location == (_crd && base === void 0 ? (_reportPossibleCrUseOfbase({
              error: Error()
            }), base) : base).BattleRoomCardLocation.Terrain) {
              let terrainData = this.copyAreas[dragCardData.dragAreaIdx];
        ...
        updateAll() {
          //刷新全部格子
          for (let index = 0; index < (_crd && battleLogicMgr === void 0 ? (_reportPossibleCrUseOfbattleLogicMgr({
            error: Error()
          }), battleLogicMgr) : battleLogicMgr).handGridList.length; index++) {
            let battleCardGrid = (_crd && battleLogicMgr === void 0 ? (_reportPossibleCrUseOfbattleLogicMgr({
              error: Error()
          ...
          for (let areaIdx = 0; areaIdx < (_crd && battleLogicMgr === void 0 ? (_reportPossibleCrUseOfbattleLogicMgr({
            error: Error()
