  ...
  return {
    ...
    execute: function () {
      ...
      _export("CardInfoPanel", CardInfoPanel = class CardInfoPanel extends (_crd && Panel === void 0 ? (_reportPossibleCrUseOfPanel({
        ...
        flush(data, type, callback, isShowUp = false) {
        ...
        closeUpgradeResult() {
          ...
          if (this.curLvResult) {
            ...
            if (this.curLvResult.levelAfter > this.curLvResult.levelBefore) {
              let lvAwardList = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
                error: Error()
        ...
        initCardClass(id, cardId) {
          ...
          for (let std of cards) {
            ...
            if (!isGet) {
              let cardClassData = (_crd && PlayerData === void 0 ? (_reportPossibleCrUseOfPlayerData({
                error: Error()
        ...
        updateCardCalssCell(cell, cellInfo) {
          let cellCom = cell.getComponent(_crd && CardClassItem === void 0 ? (_reportPossibleCrUseOfCardClassItem({
            error: Error()
        ...
        initUpgradeHaveItem() {
          ...
          let maxLen = Math.max(this.upgradeHaveItemCont.children.length, itemIds.length);
          let node;
          let haveItem;
        ...
        updateAwardCardCell(cell, cellInfo) {
          let cellCom = cell.getComponent(_crd && CardAwardItem === void 0 ? (_reportPossibleCrUseOfCardAwardItem({
            error: Error()
        ...
        updateShowQual() {
          ...
          let itemIdMap = {};
          let addExp = 0;
          let curQual = this.selectCardData.quality;
          let stdUpgrade;
          ...
          for (let index = 0; index < this.upgradeList.length; index++) {
            ...
            if (stdUpgrade.Quality > curQual && stdUpgrade.Quality <= this.targetQual) {
              for (let j = 0; j < stdUpgrade.CostItemIds.length; j++) {
                let id = stdUpgrade.CostItemIds[j];
                let num = stdUpgrade.CostItemNums[j];
                let itemData = itemIdMap[id];
        ...
        showUpgradeResult(cardData, change) {
          let oldCardData = (_crd && PlayerData === void 0 ? (_reportPossibleCrUseOfPlayerData({
            error: Error()
          ...
          let maskOpacity = this.upgradeResultMaskBtn.node.getComponent(UIOpacity);
          ...
          tween(this.upgradeResultCard.node).to(0.5, {
            ...
            let animNam = `LVUP_${cardData.quality}`;
