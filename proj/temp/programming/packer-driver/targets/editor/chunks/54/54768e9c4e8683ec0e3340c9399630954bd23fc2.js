      ...
      _export("FightUI", FightUI = class FightUI extends (_crd && Panel === void 0 ? (_reportPossibleCrUseOfPanel({
        ...
        updateArea(datas, round) {
          for (let i = 0; i < datas.length; i++) {
            ...
            let cont = area.getChildByName("cont");
            ...
            if (!data) {
              ...
              let descLab = cont.getChildByName("descLab").getComponent(RichText);
              let icon = cont.getChildByName("icon").getComponent(Sprite);
              let nameLab = cont.getChildByName("nameLab").getComponent(Label);
              let iconName = "area_icon_none";
              descLab.string = "";
              nameLab.string = "未揭示";
              ...
              let labTrans = descLab.node.getComponent(UITransform);
        ...
        onShowBattleDoubleEffect(worldPos) {
          let showPos = (_crd && GameSet === void 0 ? (_reportPossibleCrUseOfGameSet({
            error: Error()
        ...
        showSectionSettle() {
          let data = this.sectionSettleDataMap ? this.sectionSettleDataMap[this.roomData.roomId] : null;
        ...
        showSatrElement() {
          let data = this.starElementSettleDataMap ? this.starElementSettleDataMap[this.roomData.roomId] : null;
