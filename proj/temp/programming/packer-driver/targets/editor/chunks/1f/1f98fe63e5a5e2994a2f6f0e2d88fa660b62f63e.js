System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2", "__unresolved_3", "__unresolved_4", "__unresolved_5", "__unresolved_6", "__unresolved_7", "__unresolved_8", "cc/env", "__unresolved_9", "__unresolved_10"], function (_export, _context) {
  ...
  function CountPower(roleType, level, role) {
    ...
    for (let type of std.AttrFight) {
      power = power + (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
        error: Error()
      }), CfgMgr) : CfgMgr).GetFightAttr()[type].Power * (_crd && DL === void 0 ? (_reportPossibleCrUseOfDL({
        error: Error()
      }), DL) : DL).GetFightAttrValue(type, std);
    }
    ...
    if (stdlv) {
      ...
      for (let type of stdlv.AttrFight) {
        power = power + (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
          ...
          error: Error()
        }), DL) : DL).GetFightAttrValue(type, stdlv);
      }
    }
  ...
  function RoleCardPower(roleType, roleQual = 1, roleLv = 1, role = null) {
    let val = 0;

    let countPower = (attrList, valList, attrGroup) => {
      let count = 0;
      let attrType;
      let attrVal;
    ...
    let attrGroup = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
      error: Error()
    }), CfgMgr) : CfgMgr).GetAttr();
    let fightAttrGroup = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
      error: Error()
    ...
    let countMainSkill = (skillId, skillLv) => {
      //主动技能属性计算
      let stdMainSkill = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
        error: Error()
    ...
    let countPassiveSkill = (skillId, skillLv) => {
      let passiveSkill = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
        error: Error()
    ...
    let stdRole = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
      error: Error()
    }), CfgMgr) : CfgMgr).GetRole()[roleType];
    ...
    let stdQuality = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
      error: Error()
    ...
    execute: function () {
      ...
      PlayerData = class PlayerData {
        ...
        static countDown(timestamp) {
          ...
          let leftS = Math.floor(leftTime % 60); // 将剩余时间拼接成字符串，如'12:34:56'，并返回
        ...
        static countDown2(timestamp) {
          let leftTime = timestamp - PlayerData.GetServerTime(); // 计算剩余时间的各个单位
        ...
        static WeeHoursTime(timeZone = 8) {
          let serverTime = PlayerData.GetServerTime();
          let offsetTime = serverTime % 86400; // + (timeZone * 3600);

          let curTime = serverTime - offsetTime; //let dates:string[] = DateUtils.TimestampToDate(curTime * 1000, true);
        ...
        static UpdateSpecialItems(eimt = true) {
        ...
        static getActiveTask() {
          let stdTasks = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
          ...
          let stdAllTasks = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
          }), CfgMgr) : CfgMgr).GetTask();

          for (let task of stdAllTasks) {
            //已经完成的任务
        ...
        static GetResBySubType(subType) {
          ...
          if (subType == 0) {
            ...
            roleData.forEach(data => {
              let stateList = PlayerData.GetRoleStateList(data);
        ...
        static getOrderIsLockByViewId(view_id) {
          for (let index = 0; index < this.tradeViewData.order_list.length; index++) {
            const element = this.tradeViewData.order_list[index];
        ...
        static flushWorkers(buildingId, building) {
          ...
          let works = building.workerIdArr;
        ...
        static GetRoleStateList(role) {
          let typeList = [(_crd && RoleStateType === void 0 ? (_reportPossibleCrUseOfRoleStateType({
            error: Error()
        ...
        static checkBuilding(buildingId) {
          let buildDefaine = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
        ...
        static GetBuildingLv(homeId, type, index = 0) {
          let stdDef = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
        ...
        static CheckCondition(homeId, conditionId, conditionValue) {
          for (let i = 0; i < conditionId.length; i++) {
            ...
            switch (id) {
              case 1:
                let objs = this.GetBuildingByType((_crd && BuildingType === void 0 ? (_reportPossibleCrUseOfBuildingType({
                  error: Error()
        ...
        static GetProductionIds(buildingId, finsish = true) {
          let ids = [];
          let ls = this.roleInfo.item_productions;
        ...
        static UpdateSoldier(data) {
          for (let i = 0; i < this.roleInfo.soldier_productions.length; i++) {
            ...
            if (production.id == data.soldier_id && production.building_id == data.building_id) {
              let add = (_crd && maxx === void 0 ? (_reportPossibleCrUseOfmaxx({
                error: Error()
              }), maxx) : maxx)(0, data.count);
              production.count -= data.count;
              let lv = this.GetBuildingLevel(data.building_id);
              let [num, time, cost] = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
                error: Error()
        ...
        static AllocateSolider(data) {
          for (let i = 0; i < data.length; i++) {
            ...
            let ship = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
              error: Error()
        ...
        static get FishRodType() {
          if (this.fishData) {
            let num = this.fishData.player.round_cost;
            let stdRodList = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
              error: Error()
        ...
        static FishItemRemove(list) {
          if (this.fishItems) {
            ...
            for (let index = 0; index < list.length; index++) {
              fishItem = list[index];
              let findIndex = this.fishItems.findIndex(item => item.id == fishItem.id);
        ...
        static GetFishRaceIsOpen() {
          ...
          let openLv = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
          }), CfgMgr) : CfgMgr).GetFishCommon.OpenLevel;
          let buildings = this.GetBuildingByType((_crd && BuildingType === void 0 ? (_reportPossibleCrUseOfBuildingType({
            error: Error()
        ...
        static GetFishingEquipData(id, slotId) {
          let fishHero = this.GetFishingEquipHero(id);
        ...
        static GetFishTradShipState(shipId) {
          if (this.fishTradeData && this.fishTradeData.ship && this.fishTradeData.ship.length) {
            let tempList = this.fishTradeData.ship.concat();
            tempList.sort((a, b) => {
        ...
        static GetLootRankCfg(seasonData, season_id) {
          ...
          let list_data = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
          }), CfgMgr) : CfgMgr).getPVPById(season_id);
          if (!list_data) return;
          let list_id = list_data.JackpotType;
          let stdRank = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
         ...
         * @returns 返回是否有更新
         */
        static SetNoticeDatas(list, unLogin = false) {
          this.noticeDatas = [];
          let newData;
          let newKeyVal = "";
          let oldKeyVal = unLogin ? "undefined" : (_crd && LocalStorage === void 0 ? (_reportPossibleCrUseOfLocalStorage({
            error: Error()
        ...
        static CheckFishIsOpen() {
          let data = this.GetOneOffRedPoint((_crd && OneOffRedPointId === void 0 ? (_reportPossibleCrUseOfOneOffRedPointId({
            error: Error()
          ...
          if (!this.fishOpenTimeCfg) {
            ...
            for (let index = 0; index < std.Opentime.length; index++) {
              let openTiem = std.Opentime[index];
          ...
          for (let index = 0; index < this.fishOpenTimeCfg.length; index++) {
            let timeData = this.fishOpenTimeCfg[index];
            let startS = (_crd && DateUtils === void 0 ? (_reportPossibleCrUseOfDateUtils({
              error: Error()
            }), DateUtils) : DateUtils).weeHoursTime(serverTime) + (timeData.startHour * 60 + timeData.startMinute) * 60;
            let endS = (_crd && DateUtils === void 0 ? (_reportPossibleCrUseOfDateUtils({
              error: Error()
        ...
        static CheckFishTradeIsOpen() {
          ...
          for (let index = 0; index < this.fishTradeOpenTimeCfg.length; index++) {
            let timeData = this.fishTradeOpenTimeCfg[index];
            let startS = (_crd && DateUtils === void 0 ? (_reportPossibleCrUseOfDateUtils({
              error: Error()
            }), DateUtils) : DateUtils).weeHoursTime(serverTime) + (timeData.startHour * 60 + timeData.startMinute) * 60;
            let endS = (_crd && DateUtils === void 0 ? (_reportPossibleCrUseOfDateUtils({
        ...
        static CheckRoleIsCanUp(roleId) {
          ...
          let curLv;
          let nextLv;
          let items = PlayerData.GetItemTypeDatas((_crd && ItemType === void 0 ? (_reportPossibleCrUseOfItemType({
            error: Error()
          }), ItemType) : ItemType).exp);
          let totlaExp = items.reduce((count, item) => {
          ...
          let checkRole = role => {
            ...
            if (nextLv) {
              ...
              if (nextLv.ConditionId && nextLv.ConditionId) {
                let condData = (_crd && DL === void 0 ? (_reportPossibleCrUseOfDL({
                  error: Error()
              ...
              if (nextLv.BreakItem && nextLv.BreakItem.length > 0) {
                ...
                for (let index = 0; index < nextLv.BreakItem.length; index++) {
                  ...
                  let have = PlayerData.GetItemCount(itemId);
        ...
        static CheckProductionRed(item_id) {
          ...
          for (let std of cfg) {
            let homeId = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
              error: Error()
            ...
            for (let index = 0; index < stdlst.length; index++) {
              ...
              for (let i = 0; i < costList.length; i++) {
                if (costList[i]) {
                  ...
                  let count = Math.floor(has / nums[i]); //能合成的最大数量
        ...
        static CheckRoleChip() {
          let datas = PlayerData.GetitemBySubType((_crd && ItemSubType === void 0 ? (_reportPossibleCrUseOfItemSubType({
            error: Error()
        ...
        static CheckLoop() {
          ...
          if (this.LootPlayerData.paid_refresh_count > 0) {
            if (this.LootSeasonInfo) {
              let seasonData = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
                error: Error()
        ...
        static UpdateAdvisterData(id, newCount) {
          let advisterData = this.advisterCount[id];
          let std = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
          }), CfgMgr) : CfgMgr).GetAdvister(id);
        ...
        static SetAdvisterCd(id) {
          let adData = this.GetAdvisterData(id);
          let std = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
          }), CfgMgr) : CfgMgr).GetAdvister(id);
        ...
        static AddChannelMsg(id, ...arg) {
          let msg = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
          }), CfgMgr) : CfgMgr).GetMessag(id);

          if (msg) {
            let formatCont = (_crd && ReplaceStr === void 0 ? (_reportPossibleCrUseOfReplaceStr({
              error: Error()
        ...
        static CheckAddTimeItem() {
          let itemDatas = this.GetItemTypeDatas((_crd && ItemType === void 0 ? (_reportPossibleCrUseOfItemType({
            error: Error()
        ...
        static GetSysIsOpen(id) {
          let stdSysOpen = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
        ...
        static SetOneOffRedPoint(id, redPointValue = false) {
          let data = this.oneOffRedPoint[id];
        ...
        static GetMyGuildLimit() {
          ...
          let memberData = this.MyGuild.members[this.roleInfo.player_id];
        ...
        static GetMyGuildApply() {
          let stdRole = this.GetMyGuildLimit();
        ...
        static GetGuildMeetPost(playerId, postId) {
          ...
          let memberData = this.MyGuild.members[playerId];
        ...
        static GetGuildIsHaveApply(guildId, list) {
          let applyData;
          let applyTime = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
        ...
        static GetGuildEventCont(guildEventData) {
          let stdGuildEvent = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
        ...
        static GetGuildEventStr(cont, eventId, index, val) {
          ...
          switch (eventId) {
            case (_crd && GuildEventType === void 0 ? (_reportPossibleCrUseOfGuildEventType({
              ...
              if (index == 1) {
                let stdGuildRole = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
                  error: Error()
        ...
        static GetGuildPostPrivilegeList(guildType, guildPost) {
          let list = [];
          let stdGuildType = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
        ...
        static GetMyGuildPrivilegeList() {
          ...
          let myPost = this.GetMyGuildLimit();
          if (!myPost) return [];
        ...
        static GetMyGuildPrivilegeById(id) {
          let list = this.GetMyGuildPrivilegeList();
        ...
        static GetMyGuildPrivilegeByIdToValue(id) {
          let myStd = PlayerData.GetMyGuildLimit();
          if (!myStd) return 0;
          let std = this.GetMyGuildPrivilegeById(id);
          if (!std) return 0;
        ...
        static GetMyGuildAuthority(myData, targetData) {
          ...
          let myStd = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
          }), CfgMgr) : CfgMgr).GetGuildRole(myData.role);
          let targetStd = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
        ...
        static GetIsActivateRights(equityId) {
          ...
          let val = PlayerData.rightsData.all_equities[equityId] || false;
          return val;
        }
        /**获取可繁育主卡数据 */
        ...
        static getXiLianRole() {
          ...
          for (let role of this.playerInfo.roles) {
            ...
            let is_has = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
              error: Error()
        ...
        static CorrectionRightsCardTime(data) {
          if (data && data.benefit_card && data.benefit_card.cards) {
            for (let key in data.benefit_card.cards) {
              let time = data.benefit_card.cards[key];
              let newTime = (_crd && DateUtils === void 0 ? (_reportPossibleCrUseOfDateUtils({
                error: Error()
        ...
        static GetEquityCardResidueTime(cardId) {
          ...
          let endTime = this.rightsData.benefit_card.cards[cardId];
          if (!endTime) return 0;
        ...
        static GetEquityByTypeGetCardList(type, isActivate = false) {
          let cardList = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
        ...
        static GetEquityByTypeTotalValue(type, isActivate = true) {
          let cardList = this.GetEquityByTypeGetCardList(type, isActivate);
          let stdEquity;
          let stdEquityCard;
          let value = 0;
        ...
        static GetEquityCardByTypeValue(cardId, type, isActivate = true) {
          let stdEquityCard = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
        ...
        static GetAllWorkMedal() {
          ...
          let info = PlayerData.GetBuilding((_crd && BuildingType === void 0 ? (_reportPossibleCrUseOfBuildingType({
            error: Error()
          ...
          for (let index = 0; index < info.workerIdArr.length; index++) {
            let roleData = info.workerIdArr[index];
            let cfg_medal = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
              error: Error()
        ...
        static CheckEquityMaxTabRead(tabId) {
          let stdTab = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
        ...
        static GetIsShowTodayTips(tipsId) {
          let oleTime = (_crd && LocalStorage === void 0 ? (_reportPossibleCrUseOfLocalStorage({
            error: Error()
        ...
        static GetFishBombSelfCurRoundCost() {
          ...
          if (this.fishBombData && this.fishBombData.player) {
            for (let key in this.fishBombData.player.round_cost) {
              let cost = this.fishBombData.player.round_cost[key];
              val = val.add(cost);
            }
          }
        ...
        static GetFishBombDieRoundNum() {
          ...
          if (this.fishBombData.round_info && this.fishBombData.player && this.fishBombData.player.fish_pool_id) {
            for (let key in this.fishBombData.player.fish_pool_id) {
              ...
              if (pondId > 0) {
                let stageInfo = this.fishBombData.round_info.stages[key];
        ...
        static SetWorldBossDataData(data) {
          ...
          let bossIndex = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
          }), CfgMgr) : CfgMgr).GetWorldBossComm.BossType.indexOf(data.base.boss_type);

          if (bossIndex > -1) {
            let endNum = data.base.boss_type % 2000 * 1000;
            let stdLvId = 2000000 + endNum + data.base.boss_lv;
            let stdRoleType = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
              error: Error()
            ...
            if (stdRoleType) {
              ...
              let stdAttrTypeList = stdRoleType.AttrFight.concat();
              let stdAttrValueList = stdRoleType.AttrFightValue.concat();
              let bossLvList = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
                error: Error()
        ...
        static SetWorldBossRankData(data) {
          ...
          let tempRankList = this.worldBossRankData ? this.worldBossRankData.rank_data_list.concat() : [];
          let newRankList = data.rank_data_list ? data.rank_data_list : [];
          let rankData;
          let changeIdList = [];
          ...
          let endLen = changeIdList.length;
          let changeId; //根据改变的id去重
        ...
        static CheckWorldBossNearOpneTime() {
          ...
          for (let index = 0; index < this.worldBossOpenTimeList.length; index++) {
            let timeData = this.worldBossOpenTimeList[index];
            let time = (_crd && DateUtils === void 0 ? (_reportPossibleCrUseOfDateUtils({
              error: Error()
            }), DateUtils) : DateUtils).weeHoursTime(serverTime) + (timeData.startHour * 60 + timeData.startMinute) * 60;
            let startS = time - 6000; //世界boss开始前十分钟

            let endS = time + 1000; //世界boss开始后1分钟
