      ...
      _export("SpriteLabel", SpriteLabel = (_dec = ccclass('SpriteLabel'), _dec2 = disallowMultiple(true), _dec3 = requireComponent(Layout), _dec4 = property({
        ...
        set string(value) {
          ...
          for (; i < strs.length; i++) {
            ...
            var colorValue = this.stylels.length ? this.stylels[Math.min(i, this.stylels.length - 1)].color || this.colorValue : this.colorValue;
            ch.color = new Color().fromHEX(colorValue);
            var scale = this.stylels.length ? this.stylels[Math.min(i, this.stylels.length - 1)].size || this.sizeScale : this.sizeScale;
