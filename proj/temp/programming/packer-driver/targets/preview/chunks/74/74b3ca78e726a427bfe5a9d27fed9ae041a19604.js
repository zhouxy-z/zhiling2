      ...
      _export("GameLogic", GameLogic = class GameLogic {
          ...
          for (var i = this.handCards.length - 1; i >= 0; i--) {
            ...
            if (data && data.instId == cardId) {
              if (this.myArea[area].indexOf(data) != -1) {
                ...
                var index = this.myArea[area].length;
        ...
        static getBack(cardId) {
          if (this.handCards.find(value => value.instId == cardId)) {
            ...
            var data = this.round_opreate[this.round_opreate.length - 1];

            if (data.card.instId == cardId) {
              var index = this.myArea[data.area].indexOf(data.card);
        ...
        static Enter(handCards, myArea, adArea) {
          var _this = this;

          return _asyncToGenerator(function* () {
            if (handCards) _this.handCards = handCards;
            if (myArea) _this.myArea = myArea;
            if (adArea) _this.adArea = adArea;
            (_crd && EventMgr === void 0 ? (_reportPossibleCrUseOfEventMgr({
              error: Error()
            }), EventMgr) : EventMgr).on(_crd && Battle_HandCard_To_Area === void 0 ? (_reportPossibleCrUseOfBattle_HandCard_To_Area({
              error: Error()
            }), Battle_HandCard_To_Area) : Battle_HandCard_To_Area, _this.playCard, _this);
            (_crd && EventMgr === void 0 ? (_reportPossibleCrUseOfEventMgr({
              error: Error()
            }), EventMgr) : EventMgr).on(_crd && Battle_Area_to_HandCard === void 0 ? (_reportPossibleCrUseOfBattle_Area_to_HandCard({
              error: Error()
            }), Battle_Area_to_HandCard) : Battle_Area_to_HandCard, _this.getBack, _this); // 初始手牌

            for (var data of handCards) {
              (_crd && GameScene === void 0 ? (_reportPossibleCrUseOfGameScene({
                error: Error()
