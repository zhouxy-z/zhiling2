    ...
    execute: function () {
      ...
      _export("Bezier", Bezier = class Bezier {
        ...
        static create(start, end, ctrls) {
          var ratio = Vec3.distance(start, end) / Vec3.distance(ctrls[0], ctrls[ctrls.length - 1]);
          var bezier = this.pool.length ? this.pool.pop() : new Bezier();
          ...
          for (var i = 1; i < ctrls.length - 1; i++) {
            var v = ctrls[i];
            var c = Bezier.poolv.length ? Bezier.poolv.pop() : v3();
