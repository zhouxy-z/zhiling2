    ...
    execute: function () {
      ...
      _export("GameSet", GameSet = class GameSet {
        static addTick(cb, thisObj) {
          var result = this.handles.find(obj => obj.func == cb && obj.thisObj == thisObj);
          if (result) return;
          this.handles.push({
            func: cb,
            thisObj: thisObj
        ...
        static removeTick(cb, thisObj) {
          var index = this.handles.findIndex(obj => obj.func == cb && obj.thisObj == thisObj);
          if (index == -1) return;
          this.handles.splice(index, 1);
        }

        static update(dt) {
          for (var obj of this.handles) {
