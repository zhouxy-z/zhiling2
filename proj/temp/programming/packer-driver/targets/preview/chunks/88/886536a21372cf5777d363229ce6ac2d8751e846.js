    ...
    execute: function () {
      ...
      _export("BattleCompMgr", BattleCompMgr = class BattleCompMgr {
        ...
        addComp(className, type) {
          var comps = this.compPool.get(type) || [];
          var comp = comps.pop();

          if (!comp) {
            var ctor = js.getClassByName(className);
            comp = new ctor();
        ...
        removeComp(comp) {
          var activateIndex = this.runCompList.indexOf(comp);

          if (activateIndex > -1) {
            this.runCompList.splice(activateIndex, 1);
          }

          var comps = this.compPool.get(comp.type) || [];
          comps.push(comp);
        ...
        stopComp() {
          for (var index = 0; index < this.runCompList.length; index++) {
            var comp = this.runCompList[index]; // if (checkCondCall(comp.type)) continue;
        ...
        update(dt) {
          for (var comp of this.runCompList) {
            comp.update(dt);
          }
        ...
        runBattle() {
          ...
          if (battleOpData) {
            ...
            this.isRun = true;
            var self = this;
            var comp = this.addComp(battleOpData.comp, battleOpData.type);
