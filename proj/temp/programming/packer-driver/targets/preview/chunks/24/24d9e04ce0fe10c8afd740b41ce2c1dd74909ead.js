      ...
      _export("FightLogPanel", FightLogPanel = class FightLogPanel extends (_crd && Panel === void 0 ? (_reportPossibleCrUseOfPanel({
        ...
        onLoad() {
          ...
          this.logList.updateCell = (cell, cellInfo) => {
            ...
            var cellItem = cell.getComponent(_crd && FightLogItem === void 0 ? (_reportPossibleCrUseOfFightLogItem({
              error: Error()
          ...
          this.playList.updateCell = (cell, cellInfo) => {
            var playerData = this.curSelectData.createContext.players[cellInfo.index];
            ...
            for (var index = 0; index < playerData.cards.length; index++) {
              var card = playerData.cards[index];
        ...
        onShow() {
          ...
          var datas = (_crd && LocalStorage === void 0 ? (_reportPossibleCrUseOfLocalStorage({
            error: Error()
          ...
          switch (btn) {
            ...
            case this.infoBtn:
              if (this.curSelectData && this.selectPlayer) {
                ...
                _data.replayRoomId = this.curSelectData.createContext.roomId;
                _data.beginRound = 1;
                _data.viewPlayerId = this.selectPlayer.playerId;
                (_crd && Session === void 0 ? (_reportPossibleCrUseOfSession({
                  error: Error()
                }), Session) : Session).Send((_crd && Route === void 0 ? (_reportPossibleCrUseOfRoute({
                  error: Error()
                }), Route) : Route)["battle_mgr.protocol.createbattlereplayroom"], _data);
                console.log("以玩家" + _data.viewPlayerId + "视角创建战报房间---->" + _data.replayRoomId);
              }
            ...
            case this.logicInfoBtn:
              if (this.curSelectData && this.selectPlayer) {
                var _data2 = new (_crd && Req === void 0 ? (_reportPossibleCrUseOfReq({
                  error: Error()
                }), Req) : Req)["battle_mgr.protocol.createbattlereplayroom"]();

                _data2.replayRoomId = this.curSelectData.createContext.roomId;
                _data2.beginRound = 1;
                _data2.logicReplay = true;
                _data2.viewPlayerId = this.selectPlayer.playerId;
                (_crd && Session === void 0 ? (_reportPossibleCrUseOfSession({
                  error: Error()
                }), Session) : Session).Send((_crd && Route === void 0 ? (_reportPossibleCrUseOfRoute({
                  error: Error()
                }), Route) : Route)["battle_mgr.protocol.createbattlereplayroom"], _data2);
                console.log("以玩家" + _data2.viewPlayerId + "视角创建战报房间---->" + _data2.replayRoomId);
              }
