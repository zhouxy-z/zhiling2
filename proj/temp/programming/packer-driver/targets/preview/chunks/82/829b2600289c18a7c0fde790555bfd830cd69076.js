  ...
  return {
    ...
    execute: function () {
      ...
      _export("Shake", Shake = class Shake {
        ...
        update(dt) {
          if (this.tick < this.endTick) {
            ...
            if (this.loop >= this.rate) {
              this.loop = 0;
              var dx = math.randomRange(-this.power, this.power);
              var dy = math.randomRange(-this.power, this.power);
              var dz = math.randomRange(-this.power, this.power);
              (_crd && GameSet === void 0 ? (_reportPossibleCrUseOfGameSet({
                error: Error()
        ...
        static Start(power, duration, rate) {
          if (rate === void 0) {
            rate = 0.02;
          }
