      ...
      _export("GatePass", GatePass = (_dec = ccclass('GatePass'), _dec2 = disallowMultiple(true), _dec(_class = _dec2(_class = class GatePass extends (_crd && AsyncComponent === void 0 ? (_reportPossibleCrUseOfAsyncComponent({
        ...
        Show(data) {
          ...
          return _asyncToGenerator(function* () {
            ...
            var cur_lv = 21;
            _this.Lv.string = cur_lv + "";
            var seaid = data.SeaId;
            var cur_exp = 0;
            var max_exp = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
              error: Error()
            }), CfgMgr) : CfgMgr).getSeaGatepassNextExp(cur_lv);
            _this.curProgress.string = cur_exp + "/";
            _this.allProgress.string = max_exp + "";
            _this.proBar.progress = cur_exp / max_exp;
            var seapass = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
              error: Error()
            }), CfgMgr) : CfgMgr).getSeaGatepass(seaid);
            var BgTexture = seapass[0].BgTexture ? seapass[0].BgTexture : "rightsBg1";
            var BigPrizeIcon = seapass[0].BigPrizeIcon ? seapass[0].BigPrizeIcon : "icon";
            var TitleIcon = seapass[0].TitleIcon ? seapass[0].TitleIcon : "nameIcon_10001";
            ...
            (_crd && ResMgr === void 0 ? (_reportPossibleCrUseOfResMgr({
              ...
              _this.title.spriteFrame = res;
            });
            var lv_cfg = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
              error: Error()
            }), CfgMgr) : CfgMgr).getSeaGatepassReward(seaid);

            for (var i = 0; i < seapass.length; i++) {
              var element = seapass[i];

              if (element.Grate == 1) {
                _this.baseRewardList = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
                  error: Error()
                }), CfgMgr) : CfgMgr).getSeaGatepassReward(element.ID);
              }

              if (element.Grate == 2) {
                _this.heighRewardList = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
                  error: Error()
                }), CfgMgr) : CfgMgr).getSeaGatepassReward(element.ID);
              }
            }

            _this.baseRewardList.sort((a, b) => {
              return a.Lv - b.Lv;
            });

            _this.heighRewardList.sort((a, b) => {
              return a.Lv - b.Lv;
            });

            _this.ScrollView.UpdateDatas(_this.baseRewardList);
          })();
        }

        updateItem(item, data, index) {
          var cur_show_index = 0;
          ...
          if (cur_show_index != this.cur_show_index) {
            ...
            var cur_data_index = this.cur_show_index <= this.baseRewardList.length ? this.cur_show_index - 1 : this.baseRewardList.length - 1;
            this.setStageReward(this.item, this.baseRewardList[cur_data_index], cur_data_index);
          }

          this.old_index = index;
          var lv = item.getChildByName("lv").getComponent(Label);
          var base_node = item.getChildByName("free");
          var height_node1 = item.getChildByPath("no_free/Node1");
          var height_node2 = item.getChildByPath("no_free/Node2");
          var card_node = item.getChildByPath("no_free/card");
          lv.string = "Lv." + data.Lv;
          var reward_lsit = (_crd && ItemUtil === void 0 ? (_reportPossibleCrUseOfItemUtil({
            error: Error()
          }), ItemUtil) : ItemUtil).GetSThingList(data.RewardType, data.RewardID, data.RewardNum);
          this.setItem(base_node, reward_lsit[0], 0);
          var heigh_data = this.heighRewardList[index];
          var is_has_card = heigh_data.RewardType.indexOf((_crd && ThingType === void 0 ? (_reportPossibleCrUseOfThingType({
            error: Error()
          ...
          if (is_has_card == -1) {
            var height_reward_lsit = (_crd && ItemUtil === void 0 ? (_reportPossibleCrUseOfItemUtil({
              error: Error()
        ...
        setItem(item, reward, is_state) {
          var base_item = item.getChildByName("item");
          var mask = item.getChildByName("mask").getComponent(Sprite);
          var mask_opac = item.getChildByName("mask").getComponent(UIOpacity);
          var select = item.getChildByName("select");
          var lock = item.getChildByName("lock");
          var bagItem = base_item.getComponent(_crd && BagItem === void 0 ? (_reportPossibleCrUseOfBagItem({
            error: Error()
          ...
          var _color = is_state ? "#EDF5FE" : "#000000";

          mask.color.fromHEX(_color);

          var _opacity = is_state ? 255 : 179;
        ...
        setCard(item, reward, is_state) {
          var card = item.getChildByName("Cards");
          var mask = item.getChildByName("mask").getComponent(Sprite);
          var mask_opac = item.getChildByName("mask").getComponent(UIOpacity);
          var select = item.getChildByName("select");
          var lock = item.getChildByName("lock");
