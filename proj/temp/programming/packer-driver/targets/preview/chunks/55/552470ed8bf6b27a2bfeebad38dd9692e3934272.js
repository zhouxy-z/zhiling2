      ...
      _export("Out_PlayerPower", Out_PlayerPower = class Out_PlayerPower extends (_crd && BattleBaseComp === void 0 ? (_reportPossibleCrUseOfBattleBaseComp({
        ...
        start() {
          if (this.data.terrainPower) {
            var playerPowers = this.data.terrainPower;
            var playerPower;
            var isPlayerA;

            for (var key in playerPowers) {
              ...
              for (var areaIdx = 0; areaIdx < playerPower.basePower.length; areaIdx++) {
                var powerAttr = playerPower.basePower[areaIdx];
                var cardMulAttr = playerPower.cardMul[areaIdx];
                var cardTotalPower = (_crd && battleDataMgr === void 0 ? (_reportPossibleCrUseOfbattleDataMgr({
                  error: Error()
                }), battleDataMgr) : battleDataMgr).getPlayerTerrainCardTotalPow(key, areaIdx, false);
                var newPower = cardMulAttr.finalValue * cardTotalPower + powerAttr.finalValue;
