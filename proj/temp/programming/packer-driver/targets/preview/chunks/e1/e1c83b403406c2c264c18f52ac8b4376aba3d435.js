    ...
    execute: function () {
      ...
      _export("SeasonSettlePanel", SeasonSettlePanel = class SeasonSettlePanel extends (_crd && Panel === void 0 ? (_reportPossibleCrUseOfPanel({
        ...
        showSection() {
          ...
          var base = 1;
          var doubleCont = this.data.rankRecord.doublingTimes;
          ...
          var addPoint = this.data.rankRecord.pointAfter - this.data.rankRecord.pointBefore;

          if (addPoint > 0) {
            var posA = v3(this.doubleCont.worldPosition.x, this.doubleCont.worldPosition.y + 40);
            var posB = v3(this.pro.barSprite.node.worldPosition.x + this.pro.barSprite.node.getComponent(UITransform).width, this.pro.barSprite.node.worldPosition.y);
        ...
        creatFlyPoint(posA, posB, num) {
          var pos1 = new Vec3(posA.x + 100, posA.y - 200);
          var pos2 = new Vec3(posA.x + 200, posA.y + 100);
          var pos3 = new Vec3(posA.x + 300, posA.y + 200);
          this.flyNum = num;
          var flyNode;
        ...
        flyPoint(node, delay, posA, posB, partPos) {
          var bezier = (_crd && Bezier === void 0 ? (_reportPossibleCrUseOfBezier({
            error: Error()
          }), Bezier) : Bezier).create(posA, posB, partPos);

          var update = (target, ratio) => {
            if (bezier) {
              var v = bezier.update(ratio);
        ...
        setSection(rankPoints, isChange) {
          var _this = this;
          ...
          var point = Math.max(rankPoints - curStdPvpLv.LevelPoints, 0);
          var curStar = Math.floor(point / curStdPvpLv.StarPoints);
          var maxStar = curStdPvpLv.Star - 1;
          ...
          if (curStdPvpLv.Star == 1) {
            ...
            var bright = starNode.getChildByName("bright").getComponent(sp.Skeleton);
            var isShowEffect = isChange && curStar > 0 && curStar > this.oldStar;
            ...
            var _loop = function _loop() {
              var starNode = _this.starCont.children[index];

              if (index < maxStar) {
                starNode.active = true;

                var _bright = starNode.getChildByName("bright").getComponent(sp.Skeleton);

                var _isShowEffect = isChange && !_bright.node.active;

                _bright.node.active = curStar > index;

                if (_isShowEffect) {
                  _bright.clearAnimation();

                  _bright.setAnimation(0, "show", false);

                  _bright.setCompleteListener(() => {
                    _bright.setAnimation(0, "loop", true);
                  });
                } else {
                  _bright.setAnimation(0, "loop", true);
                }
          ...
          var curExp = point % curStdPvpLv.StarPoints;
          var mapExp = curStdPvpLv.StarPoints;
        ...
        setNewSection() {
          ...
          for (var index = 0; index < addPoint; index++) {
            var point = this.data.rankRecord.pointBefore + (index + 1);
        ...
        showAward() {
          ...
          var awardItem = this.data.rewardThings[0].item;
          ...
          if (stdItem && stdItem.Itemtpye == (_crd && ItemType === void 0 ? (_reportPossibleCrUseOfItemType({
            ...
            var cardDatas = this.data.rankRecord.battlePlayers[this.playerId].cards;
          ...
          if (this.upCardData) {
            ...
            var nextUpgrade = upgradeList[this.upCardData.quality];
            var costItemId;
            var costItemNum;
