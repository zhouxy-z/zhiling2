  ...
  return {
    ...
    execute: function () {
      ...
      _export("ShopLuckyItem", ShopLuckyItem = class ShopLuckyItem extends Component {
        ...
        updateAdCdTime() {
          var adId = this.std.shopStd.AdTime; // if (adId > 0) {
        ...
        updateAddTime() {
          ...
          if (this.data && this.data.freeX1Count > 0) {
            ...
            var adId = this.std.shopStd.AdTime;
        ...
        onBtnClick(btn) {
          switch (btn) {
            case this.oneBtn:
              ...
              var shop_luck_1 = new (_crd && Req === void 0 ? (_reportPossibleCrUseOfReq({
                error: Error()
            ...
            case this.tenBtn:
              ...
              if (this.data.freeX10Count < 1) {
                var _needCount = this.std.shopStd.ConsumeX1Cost[0] * 10; // if(this.std.shopStd.ConvertToConsumeType && this.std.shopStd.ConvertToConsumeType.length && this.std.shopStd.ConsumeX1Type[0] != this.std.shopStd.ConvertToConsumeType[0]){
                ...
                if (!(_crd && ItemUtil === void 0 ? (_reportPossibleCrUseOfItemUtil({
                  error: Error()
                }), ItemUtil) : ItemUtil).CheckThingConsumes(null, this.std.shopStd.ConsumeX1ItemId, [_needCount], true)) {
                  return;
                } // }

              }

              var shop_luck_10 = new (_crd && Req === void 0 ? (_reportPossibleCrUseOfReq({
                error: Error()
              }), Req) : Req)["shop.protocol.dolucky"]();
            ...
            case this.oneHundredBtn:
              ...
              var needCount = this.std.shopStd.ConsumeX1Cost[0] * 100; // if(this.std.shopStd.ConvertToConsumeType && this.std.shopStd.ConvertToConsumeType.length && this.std.shopStd.ConsumeX1Type[0] != this.std.shopStd.ConvertToConsumeType[0]){
              //     if (!ItemUtil.CheckThingConsumes(this.std.shopStd.ConsumeX1Type, this.std.shopStd.ConsumeX1ItemId, [needCount], false)) {
              //         // let haveNum: number = ItemUtil.GetHaveThingNum(this.std.shopStd.ConsumeX1Type[0], this.std.shopStd.ConsumeX1ItemId[0]);
              //         // ShopLuckyBuyPanel.Show(this.std, 10, needCount - haveNum);
              //         // return;
              ...
              var shop_luck_100 = new (_crd && Req === void 0 ? (_reportPossibleCrUseOfReq({
                error: Error()
              }), Req) : Req)["shop.protocol.dolucky"]();
        ...
        updateShow() {
          ...
          var url = path.join("sheets/shop", this.std.shopStd.LuckyBgRes, "spriteFrame");
          ...
          if (stdPool) {
            for (var i = 0; i < stdPool.Target.length; i++) {
              var index = stdPool.Target[i];
              var itemType = stdPool.RewardType[index];
              var itemId = stdPool.RewardId[index];
              var itemNum = stdPool.RewardNumber[index];
          ...
          if (this.data && this.data.freeX10Count > 0) {
            ...
            var _item = (_crd && ItemUtil === void 0 ? (_reportPossibleCrUseOfItemUtil({
              error: Error()
            }), ItemUtil) : ItemUtil).CreateThing((_crd && ThingTypeName === void 0 ? (_reportPossibleCrUseOfThingTypeName({
              error: Error()
            }), ThingTypeName) : ThingTypeName).Item, this.std.shopStd.ConsumeX1ItemId[0], this.std.shopStd.ConsumeX1Cost[0] * 10);

            this.tenConsumeItem.SetData(_item);
          }
        }

        updateAwardItem(item, data) {
          var awardItem = item.getComponent(_crd && AwardItem === void 0 ? (_reportPossibleCrUseOfAwardItem({
            error: Error()
          }), AwardItem) : AwardItem);
