      ...
      _export("FriendHelpItem", FriendHelpItem = class FriendHelpItem extends Component {
        ...
        setData(data, info) {
          ...
          return _asyncToGenerator(function* () {
            ...
            if (_this.time) {
              ...
              _this.time = setInterval(_this.updateLayer.bind(_this), 20);
            }

            _this.pos = data.assistInfo.slot;
            var is_lock = data.islock;
            _this.addBtn.active = is_lock;
            _this.lock.active = !is_lock;
            _this.unlockNode.active = is_lock;
            _this.lockNode.active = !is_lock;

            if (info && info.assist_roles_slots < data.all_unlock_coun) {
              _this.tips.active = true;
              _this.unlockBtn.active = false;
            } else {
              _this.tips.active = false;
              _this.unlockBtn.active = true;
            }
            ...
            if (is_lock) {
              ...
              _this.spine.node.active = false;
              _this.quality.color = new Color().fromHEX((_crd && quality_color === void 0 ? (_reportPossibleCrUseOfquality_color({
                error: Error()
              }), quality_color) : quality_color)[0]);

              if (data.role) {
                _this.minCount = cfg.CostRangeMin[data.role.quality - 1];
                _this.maxCount = cfg.CostRangeMax[data.role.quality - 1];
                _this.spine.node.active = true;

                _this.setRole(data.role);
              ...
              _this.cost_num.string = _cost_num + "";
              _this.select_fee = _cost_num; //今日收益

              var shownum = (_crd && ToFixed === void 0 ? (_reportPossibleCrUseOfToFixed({
                error: Error()
              }), ToFixed) : ToFixed)(_daily_income, 2);
              _this.income_num.string = shownum;
            } else {
              _this.unlock_cost.string = data.lockCost + "";
              _this.unlock_cost_num = data.lockCost;
            }

            _this.all_unlock_coun = data.all_unlock_coun;
            _this.help_num.string = data.assistInfo.daily_assist_count + "";
            _this.Slider.enabled = _this.maxCount > 0;

            _this.updateCostProgress();
          })();
        }
        ...
        setRole(role_data) {
          ...
          return _asyncToGenerator(function* () {
            ...
            _this3.quality.color = new Color().fromHEX((_crd && quality_color === void 0 ? (_reportPossibleCrUseOfquality_color({
              error: Error()
            ...
            _this3.spine.node.active = true;
            var scale = std.Scale || 1;

            _this3.spine.node.setScale(0.4 * scale, 0.4 * scale);

            _this3.spine.skeletonData = yield (_crd && ResMgr === void 0 ? (_reportPossibleCrUseOfResMgr({
              error: Error()
            }), ResMgr) : ResMgr).LoadResAbSub(path.join("spine/role_p", std.Prefab, std.Prefab), sp.SkeletonData);

            _this3.spine.setAnimation(0, "Idle", true);

            _this3.lv.node.active = true;
            _this3.lv.string = "Lv" + role_data.level;
            _this3.role = role_data;
          })();
        }
        ...
        onUnlockBtn() {
          var hasitem = (_crd && PlayerData === void 0 ? (_reportPossibleCrUseOfPlayerData({
            error: Error()
