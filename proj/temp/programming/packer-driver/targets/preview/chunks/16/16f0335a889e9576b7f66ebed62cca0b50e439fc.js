          ...
          return _asyncToGenerator(function* () {
            ...
            if (_this.data && _this.data.card) {
              ...
              if ((_crd && battleDataMgr === void 0 ? (_reportPossibleCrUseOfbattleDataMgr({
                error: Error()
              }), battleDataMgr) : battleDataMgr).isPlayerA(_this.data.toTerrainPlayerId)) {
                //回放模式
                if ((_crd && battleDataMgr === void 0 ? (_reportPossibleCrUseOfbattleDataMgr({
                  error: Error()
                  ...
                  yield _this.scene.AutoPlayHand(_this.data.card, cardLocation.areaIdx, cardLocation.cardIdx);
                  if (_this.isExit) return;
                  yield _this.scene.CloseCard(_this.data.card);
                  if (_this.isExit) return;
                  console.log("--------->盖牌完成");
                  _this.actionEnd = true;
                } else {
                  yield _this.scene.CloseCard(_this.data.card);
                  if (_this.isExit) return;
                  console.log("--------->盖牌完成");
                ...
                if (cardLocation) {
                   ...
                   */
                  yield _this.scene.SendAdCardToArea(_this.data.card, cardLocation.areaIdx, cardLocation.cardIdx);
                  if (_this.isExit) return; // await Second(1);
