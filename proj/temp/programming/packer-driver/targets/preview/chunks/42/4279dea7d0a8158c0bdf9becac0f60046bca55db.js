    ...
    execute: function () {
      ...
      _export("MailSendPanel", MailSendPanel = class MailSendPanel extends (_crd && Panel === void 0 ? (_reportPossibleCrUseOfPanel({
        ...
        onEditBoxChanged(editBox) {
          ...
          for (var i = 0; i < editBox.string.length; i++) {
        ...
        onOpen() {
          var is_open = this.ListScrollView.node.active;
          var scaleY = is_open ? -1 : 1;
        ...
        onShowSurePanel() {
          ...
          var min_str = "ItemMin";
          var max_str = "ItemMax";
          var name = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
          ...
          var count = Number(this.EditBox.string);
          var cost_count = count;
        ...
        onTextChanged() {
          var count = Number(this.keepNumbersAndOneDot(this.EditBox.string));
          this.EditBox.string = "" + Math.ceil(count * 100) / 100;
        }

        keepNumbersAndOneDot(str) {
          // 使用正则表达式来提取所有数字和小数点
          var result = str.replace(/[^0-9.]/g, ''); // 找到第一个小数点的位置

          var firstDotIndex = result.indexOf('.'); // 如果找到了小数点，移除其后的所有小数点
        ...
        onShow() {
          ...
          var cost = 0; // if (PlayerData.GetIsActivateRights(StdEquityId.Id_3) || PlayerData.GetIsActivateRights(StdEquityId.Id_4)) {
        ...
        flush(data) {
          ...
          this.onOpen();
          var list_item_data = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
            error: Error()
        ...
        updateItem(item, data) {
          ...
          var label = item.getChildByName("label").getComponent(Label);
