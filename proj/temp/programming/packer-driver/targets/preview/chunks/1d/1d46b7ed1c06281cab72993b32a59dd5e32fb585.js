      ...
      _export("BagModule", BagModule = class BagModule {
        ...
        onItemChange(data) {
          for (var i = 0; i < (_crd && PlayerData === void 0 ? (_reportPossibleCrUseOfPlayerData({
            ...
            var item = (_crd && PlayerData === void 0 ? (_reportPossibleCrUseOfPlayerData({
              error: Error()
        ...
        getOpenBoxReward(data) {
          if (data) {
            var reward_data = [];
            var maps = {};
            var count = data.reward_thing_count.length;

            for (var index = 0; index < count; index++) {
              var awardList = void 0;
              var type = data.reward_thing_type[index];
              var id = data.reward_thing_id[index];
              ...
              if (maps[type + "_" + id]) {
                ...
                var passive_skills = [];
                var cfg_role_type = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
                  error: Error()
                }), CfgMgr) : CfgMgr).GetRole()[cfg.RoleType];

                if (cfg_role_type && cfg_role_type.PassiveGife) {
                  var passive_skill_data1 = {
                ...
                var cfg_reward_role = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
                  error: Error()
                }), CfgMgr) : CfgMgr).GetRewardRoleById(cfg.RoleType);

                if (cfg_reward_role && cfg_reward_role.PassiveId) {
                  var num = cfg_reward_role.PassiveId.length;

                  for (var _index = 0; _index < num; _index++) {
                    var element = cfg_reward_role.PassiveId[_index];
                    var passive_skill_data3 = {
                      skill_id: element,
                      level: 1
                    };
...
//# sourceMappingURL=1d46b7ed1c06281cab72993b32a59dd5e32fb585.js.map