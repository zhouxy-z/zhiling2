  ...
  return {
    ...
    execute: function () {
      ...
      _export("OpenBoxPage", OpenBoxPage = class OpenBoxPage extends Component {
        ...
        SetData(data, count, limit, is_select, callback, is_card, page_num, select_num) {
          var _this = this;
          ...
          return _asyncToGenerator(function* () {
            ...
            if (is_card) {
              ...
              _this.updateCard(data);
            }

            _this.befor_selects = select_num;
        ...
        updateItem(data) {
          ...
          return _asyncToGenerator(function* () {
            for (var index = 0; index < data.length; index++) {
              ...
              toggle.isChecked = false; // bg.spriteFrame = await ResMgr.LoadResAbSub(data[index].resData.iconBgUrl, SpriteFrame);
        ...
        updateCard(data) {
          ...
          return _asyncToGenerator(function* () {
            for (var index = 0; index < data.length; index++) {
              var item = instantiate(_this3.item);
              var bg = item.getChildByName("bg").getComponent(Sprite);
              var icon = item.getChildByPath("bg/icon").getComponent(Sprite);
              var num = item.getChildByPath("bg/num").getComponent(Label);
              var mask = item.getChildByPath("mask").getComponent(UIOpacity);
              ...
              var pos = index + _this3.page_num * 8;

              if (_this3.befor_selects.indexOf(pos) != -1) {
                toggle.isChecked = true;
              }

              toggle.node.on("toggle", _this3.onSelect, _this3);
              bg.spriteFrame = yield (_crd && ResMgr === void 0 ? (_reportPossibleCrUseOfResMgr({
                ...
                error: Error()
              }), CardQuality) : CardQuality)[data[index].CardQuality] + "_bag_bg", "spriteFrame"), SpriteFrame);
              num.string = "1";
              id.string = data[index].CardID + "_" + (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
                error: Error()
              }), CfgMgr) : CfgMgr).GetCard(data[index].CardID).CardName;
              icon.node.active = false;
              id.node.active = true;
              max_num.string = _this3.maxCount + "";
              mask.opacity = _this3.isSelect ? 0 : 255;
              toggle.interactable = _this3.isSelect;

              _this3.Layout.node.addChild(item);
            }
          })();
        }
        ...
        onSelect(toggle) {
          ...
          return _asyncToGenerator(function* () {
            if (!_this4.limit || !_this4.isSelect) return;
            yield (_crd && Second === void 0 ? (_reportPossibleCrUseOfSecond({
              error: Error()
            }), Second) : Second)(0);
            var selects = [];
            var chidlren = _this4.Layout.node.children;
            var num = 0 + _this4.befor_selects.length;
            console.log(_this4.befor_selects); // for (let child of chidlren) {

            var isSelect = toggle.isChecked;
            ...
            if (isSelect) {
              ...
              selects.push(toggle.node.getSiblingIndex() + _this4.page_num * 8);
            } else {
            ...
            if (num > _this4.limit) {
              toggle.isChecked = false;
              toggle.node.getChildByName("mask").getComponent(UIOpacity).opacity = 0;
              var index = selects.indexOf(toggle.node.getSiblingIndex() + _this4.page_num * 8);
              ...
              (_crd && Tips === void 0 ? (_reportPossibleCrUseOfTips({
                error: Error()
              }), Tips) : Tips).Show("只能选择" + _this4.limit + "个");
            }

            _this4.callBack(selects);
          })();
        }
...
//# sourceMappingURL=a3a3532a3c1ca087941ed32cc0a8d4bb7c541f0a.js.map