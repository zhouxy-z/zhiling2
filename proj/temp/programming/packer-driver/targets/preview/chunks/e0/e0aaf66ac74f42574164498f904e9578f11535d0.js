  ...
  return {
    ...
    execute: function () {
      ...
      _export("BagModule", BagModule = class BagModule {
        ...
        onItemsChange(data) {
          var item_data = (_crd && Ret === void 0 ? (_reportPossibleCrUseOfRet({
            error: Error()
          }), Ret) : Ret).PlayerItemsChangedPush.decode(data);
          console.log("道具变更后端数据推送", item_data);
          var changeMap = {};

          for (var item of item_data.list) {
            if (item.countAfter > 0) {
              var type = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
                error: Error()
        ...
        getOpenBoxReward(data) {
          var open_box_data = (_crd && Ret === void 0 ? (_reportPossibleCrUseOfRet({
            error: Error()
