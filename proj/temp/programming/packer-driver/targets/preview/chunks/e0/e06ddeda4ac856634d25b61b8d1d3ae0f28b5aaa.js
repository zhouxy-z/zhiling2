        ...
        SetData(data, is_hide_use) {
          ...
          return _asyncToGenerator(function* () {
            ...
            if (typeof data == "string") {
              _this.add.active = true;
              _this.cardNode.active = false;
              return;
            }

            if (!data) return;
            _this.nameLab.string = data.name;
            _this.warning.active = !data.size || data.size < 12;
            var url = !data.size || data.size < 12 ? path.join("sheets/cards/red", "spriteFrame") : path.join("sheets/cards/blue", "spriteFrame");
            (_crd && ResMgr === void 0 ? (_reportPossibleCrUseOfResMgr({
              error: Error()
            }), ResMgr) : ResMgr).LoadResAbSub(url, SpriteFrame, res => {
              _this.bg.spriteFrame = res;
            });
            _this.getBtn.node.active = data.id == "";
            _this.useLab.active = !is_hide_use && (_crd && PlayerData === void 0 ? (_reportPossibleCrUseOfPlayerData({
              error: Error()
            ...
            var avatarId = data.avatarId ? data.avatarId : (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
              error: Error()
            ...
            if (cfg) {
              ...
              (_crd && ResMgr === void 0 ? (_reportPossibleCrUseOfResMgr({
                ...
                _this.headIcon.spriteFrame = res;
              });
            } else {
              console.log("配置错误");
            }

            var cardBackcfg = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
              error: Error()
            }), CfgMgr) : CfgMgr).GetCardBackCfgById(data.cardBackId);

            if (cardBackcfg) {
              var callBackIcon = cardBackcfg.icon;
              (_crd && ResMgr === void 0 ? (_reportPossibleCrUseOfResMgr({
                error: Error()
