  ...
  return {
    ...
    execute: function () {
      ...
      _export("TestSkill", TestSkill = (_dec = ccclass('TestSkill'), _dec2 = requireComponent(RenderRoot2D), _dec3 = property({
        ...
        onLoad() {
          ...
          return _asyncToGenerator(function* () {
            // 处理2d组件在3d场景的深度渲染问题
            var comps = _this.getComponentsInChildren(Component);
            ...
            for (var i = 0, len = comps.length; i < len; i++) {
              ...
              if (comp instanceof sp.Skeleton || comp instanceof Sprite) {
                if (!comp.customMaterial || comp.customMaterial.uuid != (_crd && ResMgr === void 0 ? (_reportPossibleCrUseOfResMgr({
                  ...
                  comp.customMaterial = yield (_crd && ResMgr === void 0 ? (_reportPossibleCrUseOfResMgr({
                    error: Error()
            ...
            _this.node.on("tigger", _this.onTigger, _this);
          })();
        }
        ...
        get testTargetSet() {
          ...
          var my = ["FightScene/myDeck", "FightScene/myHand", "FightScene/myDrop", "FightScene/myDel", "FightScene/ui/myEnergy"];
          var ad = ["FightScene/adDeck", "FightScene/adHand", "FightScene/adDrop", "FightScene/adDel", "FightScene/ui/adEnergy"];
          var area1 = [find("FightScene/area0")['hitTest1'], find("FightScene/area1")['hitTest1'], find("FightScene/area2")['hitTest1']];
          var area2 = [find("FightScene/area0")['hitTest2'], find("FightScene/area1")['hitTest2'], find("FightScene/area2")['hitTest2']];
          var mytag = [area1],
              adtag = [area2];
        ...
        Test(e) {
          ...
          return _asyncToGenerator(function* () {
            if (e) yield (_crd && Second === void 0 ? (_reportPossibleCrUseOfSecond({
              error: Error()
            }), Second) : Second)(0.5);
            ...
            var len = _this2.testTargetSet.length;

            if (_this2.prefab && len > 1) {
              var queue = [_this2];
              var parent = _this2.node.parent;
              ...
              for (var i = 0; i < len; i++) {
                ...
                if (!skill) {
                  copy = true;
                  skill = instantiate(_this2.prefab).getComponent(TestSkill);
                ...
                var target = _this2.testTargetSet.pop(1);

                var list = skill.GetEffects();
                ...
                for (var set of list) {
                  if (set && set.root) {
                    ...
                    if (set.last && i == len - 1) {
                      set.Play(_this2.Test.bind(_this2), target, 0, copy);
                    } else {
                ...
                if (_this2.multipleDelay > 0) yield (_crd && Second === void 0 ? (_reportPossibleCrUseOfSecond({
                  error: Error()
                }), Second) : Second)(_this2.multipleDelay);
              }
            } else {
              var targets = _this2.testTargetSet.pop(1);

              _this2.GetEffects();

              var sets = [];

              for (var _set of _this2.efflist) {
                if (_set) {
                  _set.Reset();

                  if (_set.last) _set.times = 1;
              ...
              for (var _set2 of sets) {
                _set2.Play(_this2.Test.bind(_this2), targets);
              }
            }
          })();
        }
        ...
        GetEffects(parent) {
          ...
          children.forEach(value => {
            var set = value.getComponent(_crd && EffectSet === void 0 ? (_reportPossibleCrUseOfEffectSet({
              error: Error()
            }), EffectSet) : EffectSet);
            ...
            if (set.root) {
              ...
              var lst = ((_set$nextEffects = set.nextEffects) == null || _set$nextEffects.concat == null ? void 0 : _set$nextEffects.concat()) || [];
              ...
              while (lst.length) {
                ...
                if (set && !sets[set.node.uuid]) {
                  ...
                  sets[set.node.uuid] = set;
                  if ((_set3 = set) != null && (_set3 = _set3.nextEffects) != null && _set3.length) lst.push(...set.nextEffects);
                }
              }
            }
          });
          ...
          for (var k in sets) {
            var set = sets[k]; //child.getComponent(EffectSet);
      ...
      _export("EffectTarget", EffectTarget = class EffectTarget {
        ...
        get length() {
          var len = Math.max(this.targets.length, 0);
          if (len) return len;
          return Math.min(this.myArea.length, this.adArea.length, 1);
        }

        pop(skillId) {
          var myArea = this.myArea.length ? this.myArea.shift() : undefined;
          var adArea = this.adArea.length ? this.adArea.shift() : undefined;
          var target = this.targets.length ? this.targets.shift() : undefined;
          return {
