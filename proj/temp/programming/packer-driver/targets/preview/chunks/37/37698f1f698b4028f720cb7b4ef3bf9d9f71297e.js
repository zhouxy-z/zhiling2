    ...
    execute: function () {
      ...
      _export("CardsPanel", CardsPanel = class CardsPanel extends (_crd && Panel === void 0 ? (_reportPossibleCrUseOfPanel({
        ...
        setHaveCardGroup(data) {
          ...
          var getdeckcards = new (_crd && Protocol === void 0 ? (_reportPossibleCrUseOfProtocol({
            error: Error()
        ...
        updatePanel() {
          ...
          var jianju = Math.floor(yu / (num - 1)); //最右侧剩余间隙

          var offset = yu % (num - 1); //最小间隙15
        ...
        onMoreBtn(index) {
          switch (index) {
            case MoreType.del_card:
              var delete_data = new (_crd && Protocol === void 0 ? (_reportPossibleCrUseOfProtocol({
                error: Error()
          ...
          if (this.editBox.string) {
            ...
            var _loop = function _loop() {
              if (Object.hasOwnProperty.call(card_cfg, key)) {
                var iterator = card_cfg[key];

                if (iterator.CardName.includes(_this.editBox.string) || iterator.SkillDesc.includes(_this.editBox.string)) {
                  _this.card_list.forEach(item => {
                    if (item.cardId == iterator.ID) {
                      _this.search_card_list.push(item);
                    }
                  });
                }
              }
