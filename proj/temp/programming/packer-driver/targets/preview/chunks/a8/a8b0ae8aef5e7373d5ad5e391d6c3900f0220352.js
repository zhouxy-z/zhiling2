  ...
  return {
    ...
    execute: function () {
      ...
      _export("ItemTips", ItemTips = class ItemTips extends (_crd && Panel === void 0 ? (_reportPossibleCrUseOfPanel({
        ...
        flush(data, is_show_getNode) {
          var _this = this;

          return _asyncToGenerator(function* () {
            if (is_show_getNode === void 0) {
              is_show_getNode = true;
            }
            ...
            if (!std_item) return;
            _this.nameLab.string = std_item.ItemName;
            _this.cd_time.string = std_item.LockTime + "";
            _this.tipsLab.string = std_item.Remark + ""; // this.bg.spriteFrame = await ResMgr.LoadResAbSub(path.join(folder_quality, CardQuality[std_item.Quality] + "_bag_bg", "spriteFrame"), SpriteFrame);

            var spr = std_item.Icon;
            ...
            if (is_show_getNode) {
              _this.getNode.active = true;
              _this.cont.getComponent(UITransform).height = 709;
            } else {
              _this.getNode.active = false;
              _this.cont.getComponent(UITransform).height = 709 - 300;
            }

            _this.cont.children.forEach(node => {
