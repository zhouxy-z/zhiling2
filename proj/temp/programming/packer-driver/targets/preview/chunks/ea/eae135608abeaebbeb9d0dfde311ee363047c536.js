    ...
    execute: function () {
      ...
      _export("Out_TerrainToTerrain_2d", Out_TerrainToTerrain_2d = class Out_TerrainToTerrain_2d extends (_crd && BattleBaseComp === void 0 ? (_reportPossibleCrUseOfBattleBaseComp({
          ...
          if (this.data.success) {
            ...
            for (var index = 0; index < terrainAreaList.length; index++) {
              ...
              if (terrainArea.terrainData.instId == this.data.fromTerrainInstId) {
                for (var _index = 0; _index < terrainArea.gridList.length; _index++) {
                  grid = terrainArea.gridList[_index];
            ...
            for (var _index2 = 0; _index2 < terrainAreaList.length; _index2++) {
              terrainArea = terrainAreaList[_index2];

              if (terrainArea.terrainData.instId == this.data.toTerrainInstId) {
                for (var _index3 = 0; _index3 < terrainArea.gridList.length; _index3++) {
                  grid = terrainArea.gridList[_index3];
        ...
        resetAreaData() {
          ...
          for (var areaIdx = 0; areaIdx < terrainAreaList.length; areaIdx++) {
            ...
            for (var index = 0; index < terrainArea.gridList.length; index++) {
              ...
              var cardData = terrainData.cards[index] || null;
          ...
          for (var _areaIdx = 0; _areaIdx < terrainAreaList.length; _areaIdx++) {
            terrainArea = terrainAreaList[_areaIdx];
            terrainData = terrainDataList[_areaIdx];

            for (var _index4 = 0; _index4 < terrainArea.gridList.length; _index4++) {
              grid = terrainArea.gridList[_index4];

              var _cardData = terrainData.cards[_index4] || null;

              grid.setData(_cardData);
            }
          }
        }
