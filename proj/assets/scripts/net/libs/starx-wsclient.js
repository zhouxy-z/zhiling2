(function() {
  ...
  var sendMessage = function(reqId, route, msg) {
    ...
    if(!msg) {
      console.warn("未定义此协议消息，请先将proto定义绑定对应的路由消息！");
      return;
    }
  ...
  var processMessage = function(starx, msg) {
    if(!msg.id) {
      ...
      let result = starx.emit(msg.route, msg.body);
      if(!result) starx.emit("un_receive_msg",msg.route);
      return result;
    }
    console.log("<<<",msg.route);
  ...
  var deCompose = function(msg) {
    ...
    if(decodeIO_decoder && decodeIO_decoder.lookup(route)) {
      ...
      if(strdecode) {
        return strdecode(msg.body);
      }
      let code = Protocol.strdecode(msg.body);
